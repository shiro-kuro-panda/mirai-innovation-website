...
```
index.html                      apps/dream-job-storybook/ へのリダイレクト
profile/index.html              自己紹介ページ
assets/style.css                自己紹介ページのスタイル
apps/dream-job-storybook/       MirAI塾 夢の職業絵本メーカー
```

トップを開くと絵本メーカーに移動します。自己紹介ページは `/profile/` にあります。

`profile/index.html` 内の `class="tbd"` が付いた箇所はプロフィール未確定のプレースホルダーです。
